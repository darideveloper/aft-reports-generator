...
## Data Flow
1.  **Fetching Options:** `fetchOptions` in `src/lib/api/options.ts` returns a list of departments.
2.  **State Management:** `useFormStore` manages the `department` value within `emailResponse`.
3.  **UI Interaction:** `GeneralDataScreen.tsx` renders a `Dropdown` component for the department.
4.  **Validation:** `GeneralDataScreen.tsx` ensures a department is selected before proceeding.
